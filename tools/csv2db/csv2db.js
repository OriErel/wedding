...
const fileCallback = (file, data) => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    con.query(`select * from guests where first_name = "${firstName}" AND last_name = "${lastName}"`, (err, result) => {
      ...
      if (result.length) {
        console.log(
          `Already found first_name = "${firstName}" AND last_name = "${lastName}" AND group = ${group}, skipping`,
        );

        return postFileHandle(lines, linesAddedToDb);
      }

      // TODO insert guest by ID using random string for numeric and alpha chars only
      con.query(
        `insert into guests values (0,"${firstName}", "${lastName}", null, null, "${extraData}", "${group}", "${cellphone}", ${isMale}, null)`,
        err => {
          if (err) throw err;
